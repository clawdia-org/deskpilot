...
Patterns for using deskpilot effectively in multi-step desktop automation tasks.
...
deskpilot snapshot --root @e1 -i --compact
...
# deskpilot snapshot --skeleton --app "System Settings" -i --compact
# deskpilot snapshot --root @e5 -i --compact

# Found: @e3 = "Computer Name" textfield, @e5 = "Local Hostname" textfield

# Clear and fill each field
deskpilot clear @e3
deskpilot type @e3 "My MacBook Pro"
deskpilot clear @e5
deskpilot type @e5 "my-macbook-pro"
...
deskpilot snapshot --app "TextEdit" --surface menubar -i
# Found: @e1 = "File" menuitem

deskpilot click @e1
deskpilot wait --menu --app "TextEdit"
deskpilot snapshot --app "TextEdit" --surface menu -i
# Found: @e5 = "Save As..." menuitem

deskpilot click @e5

# 2. Wait for the dialog, then snapshot the SHEET surface (not the full window)
deskpilot wait --window "Save"
deskpilot snapshot --app "TextEdit" --surface sheet -i
```
...
deskpilot wait --menu-closed --app "Finder" --timeout 2000
```
...
deskpilot wait --window "Save As" --timeout 5000
...
# Fill dialog fields
deskpilot type @e2 "my-document.txt"

# Click OK/Save
deskpilot click @e5

# After dialog closes, snapshot the window again for fresh refs
deskpilot snapshot --app "TextEdit" -i
```
...
deskpilot snapshot --skeleton --app "App" -i --compact
...
deskpilot scroll @e8 --direction down --amount 5
deskpilot find --app "App" --name "Target Item"
...
deskpilot click @e1          # Focus first field
deskpilot type @e1 "value1"
deskpilot press tab
# Now in next field — type directly since focus moved
deskpilot press tab          # Skip a field
deskpilot type @e3 "value3"  # Or snapshot again to get new refs
```
...
deskpilot get @e5 --property value

# Option B: Copy via keyboard
deskpilot click @e5
deskpilot press cmd+a
...
deskpilot drag --from @e3 --to @e8

# Between coordinates
deskpilot drag --from-xy 100,200 --to-xy 500,400

# Mixed: element to coordinates
deskpilot drag --from @e3 --to-xy 500,400 --duration 500
```
...
deskpilot click @e5  # "Download" button

# Wait for completion text
deskpilot wait --text "Download complete" --app "App" --timeout 30000

# Or wait for a specific element to appear
deskpilot wait --element @e10 --timeout 10000
```
...
deskpilot snapshot --app "Calculator" -i

# Dense app → skeleton first
# deskpilot launch "Slack"
# deskpilot snapshot --skeleton --app "Slack" -i --compact
# deskpilot snapshot --root @e2 -i --compact

# ... perform automation ...

deskpilot close-app "Calculator"
```
...
deskpilot focus-window --window-id "w-5678"

# Snapshot that specific window
deskpilot snapshot --app "Finder" --window-id "w-5678" -i
```
...
deskpilot is @e6 --property checked
# If result is false, then check it
deskpilot check @e6

# Or use check/uncheck directly (they're idempotent)
deskpilot check @e6    # No-op if already checked
deskpilot uncheck @e6  # No-op if already unchecked
```
