...
# guess which of several cached `deskpilot-ffi-<hash>/` build dirs is
# current — `find target | head -1` would pick arbitrarily.

ROOT=$(git rev-parse --show-toplevel)
cd "$ROOT"

cargo build -p deskpilot-ffi >/dev/null 2>&1
...
cp "$GENERATED" crates/ffi/include/deskpilot.h
echo "Updated crates/ffi/include/deskpilot.h"
